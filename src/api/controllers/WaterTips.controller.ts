...
export class WaterTips {
    static async GetTip(req: Request, res: Response) {
        try {
            ...
            if (!result.status) {
                return res.status(404).json({
                    success: false,
                    message:
                        result.message === "Failed to fetch tip."
                            ? "No tip found."
    ...
    static async AddTip(req: Request, res: Response) {
        try {
            ...
            const result: any = await WaterTip.AddTip(tip, lang);
    ...
    static async RemoveTip(req: Request, res: Response) {
        try {
            const { password, tip, lang } = req.body;
            ...
            const result: any = await WaterTip.RemoveTip(tip, lang);
    ...
    static async ListTip(req: Request, res: Response) {
        try {
            const { password, tip, lang } = req.query as {password: string, tip: string, lang: string};
            ...
            const result: any = await WaterTip.ListTip(tip, lang);

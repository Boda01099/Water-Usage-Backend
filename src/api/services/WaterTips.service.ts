...
export class WaterTip {
  static async GetTip(language: string) {
    try {
  ...
  static async AddTip(Tip: string, lang: string) {
    try {
  ...
  static async RemoveTip(Tip: string, lang: string) {
    try {
  ...
  static async ListTip(Tip: string, lang: string) {
    try {
      const tipsArray = await WaterTipsModel.find({ language: lang }).select('Tip -_id');
      const tips = tipsArray.map((item: any) => item.Tip);
